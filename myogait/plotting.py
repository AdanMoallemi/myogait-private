...
def plot_stroboscopic_cycle(
    data: dict,
    cycles_result: Optional[dict] = None,
    side: str = "auto",
    cycle_idx: int = 0,
    n_strobe_frames: int = 8,
    figsize: Tuple[float, float] = (12, 6),
    title: Optional[str] = "Walking (Gait Cycle Progression)",
    scale_m: Optional[float] = None,
    visible_side: str = "both",
) -> matplotlib.figure.Figure:
    """Generate a stroboscopic (chronophotographic) stick-figure progression plot.

    Visualizes sequential human pose stick figures across a representative
    gait cycle along with Center of Mass (COM) trajectory and event markers
    (Heel-Contact and Toe-Off).

    Parameters
    ----------
    data : dict
        Pivot JSON dict with ``frames`` and optional ``events``.
    cycles_result : dict, optional
        Cycle segmentation result from ``segment_cycles()``.
    side : str
        ``'left'``, ``'right'``, or ``'auto'`` (selects first valid cycle).
    cycle_idx : int
        Index of the cycle to visualize (default: 0).
    n_strobe_frames : int
        Number of strobe stick figures to display across the cycle (default: 8).
    figsize : tuple
        Matplotlib figure size ``(width, height)`` in inches.
    title : str, optional
        Plot title.
    scale_m : float, optional
        Height scaling factor to convert normalized units to meters.
    visible_side : str
        ``'both'``, ``'left'``, or ``'right'``.
    ...
    if not frames:
        fig, ax = plt.subplots(figsize=figsize)
        ax.text(0.5, 0.5, "No landmark data available", ha="center", va="center")
        return fig

    events = data.get("events", {})
    valid_frames = [f for f in frames if f.get("landmarks")]
    if len(valid_frames) < 5:
        fig, ax = plt.subplots(figsize=figsize)
        ax.text(0.5, 0.5, "Insufficient valid frames for stroboscopic plot", ha="center", va="center")
        return fig

    # Determine walking direction from hip landmarks
    hip_xs = []
    for f in valid_frames:
        lm = f["landmarks"]
        lh = lm.get("LEFT_HIP", {})
        rh = lm.get("RIGHT_HIP", {})
        x_vals = [p["x"] for p in [lh, rh] if p.get("x") is not None and not np.isnan(p["x"])]
        if x_vals:
            hip_xs.append(np.mean(x_vals))

    flip_x = (hip_xs[-1] - hip_xs[0]) < 0 if len(hip_xs) > 5 else False

    # Extract cycle frames
    left_hs = [e["frame"] for e in events.get("left_hs", [])]
    right_hs = [e["frame"] for e in events.get("right_hs", [])]
    left_to = [e["frame"] for e in events.get("left_to", [])]
    right_to = [e["frame"] for e in events.get("right_to", [])]

    start_f, end_f = None, None
    if cycles_result and cycles_result.get("cycles"):
        c_list = cycles_result["cycles"]
        if side in ("left", "right"):
            c_match = [c for c in c_list if c.get("side") == side]
            if c_match:
                c_sel = c_match[min(cycle_idx, len(c_match) - 1)]
                start_f = c_sel.get("start_frame")
                end_f = c_sel.get("end_frame")
        if start_f is None and c_list:
            c_sel = c_list[min(cycle_idx, len(c_list) - 1)]
            start_f = c_sel.get("start_frame")
            end_f = c_sel.get("end_frame")

    if start_f is None or end_f is None:
        if len(left_hs) >= 2:
            start_f, end_f = left_hs[0], left_hs[1]
        elif len(right_hs) >= 2:
            start_f, end_f = right_hs[0], right_hs[1]
        else:
            start_f = valid_frames[0].get("frame_idx", 0)
            end_f = valid_frames[-1].get("frame_idx", len(valid_frames) - 1)

    cycle_frames = [f for f in valid_frames if start_f <= f.get("frame_idx", 0) <= end_f]
    if len(cycle_frames) < 5:
        cycle_frames = valid_frames

    # Ground level and height scale
    ground_ys = []
    for f in cycle_frames:
        lm = f["landmarks"]
        for k in ["LEFT_ANKLE", "RIGHT_ANKLE", "LEFT_HEEL", "RIGHT_HEEL", "LEFT_FOOT_INDEX", "RIGHT_FOOT_INDEX"]:
            pt = lm.get(k, {})
            if pt.get("y") is not None and not np.isnan(pt["y"]):
                ground_ys.append(pt["y"])
    ground_y = np.percentile(ground_ys, 95) if ground_ys else 0.9
    ...
    for f in cycle_frames:
        ...
        for k in ["NOSE", "LEFT_EYE", "RIGHT_EYE", "LEFT_EAR", "RIGHT_EAR"]:
            ...
            if pt.get("y") is not None and not np.isnan(pt["y"]):
                head_ys.append(pt["y"])
    head_y = np.percentile(head_ys, 5) if head_ys else 0.1
    body_span_norm = max(0.2, ground_y - head_y)

    if scale_m is not None and scale_m > 0:
        scale = scale_m
    else:
        scale = 1.72 / body_span_norm
    ...
    def get_xy(pt):
        ...
        if np.isnan(x) or np.isnan(y):
            return None
        x_m = (1.0 - x) * scale if flip_x else x * scale
        y_m = (ground_y - y) * scale
        return (x_m, y_m)

    first_hip = get_xy(cycle_frames[0]["landmarks"].get("LEFT_HIP") or cycle_frames[0]["landmarks"].get("RIGHT_HIP"))
    x_offset = first_hip[0] if first_hip else 0.0

    strobe_indices = np.linspace(0, len(cycle_frames) - 1, n_strobe_frames, dtype=int)

    fig, ax = plt.subplots(figsize=figsize, dpi=200)

    # 1. Plot COM trajectory curve
    com_xs, com_ys = [], []
    for f in cycle_frames:
        lm = f["landmarks"]
        lh, rh = lm.get("LEFT_HIP"), lm.get("RIGHT_HIP")
        pts = [get_xy(lh), get_xy(rh)]
        valid_pts = [p for p in pts if p is not None]
        if valid_pts:
            avg_x = np.mean([p[0] - x_offset for p in valid_pts])
            avg_y = np.mean([p[1] for p in valid_pts])
            com_xs.append(avg_x)
            com_ys.append(avg_y)

    if com_xs and com_ys:
        if len(com_xs) > 5:
            smooth_ys = gaussian_filter1d(com_ys, sigma=2.0)
            ax.plot(com_xs, smooth_ys, color="#e41a1c", linewidth=2.5, label="COM position", zorder=10)
        else:
            ax.plot(com_xs, com_ys, color="#e41a1c", linewidth=2.5, label="COM position", zorder=10)

    # 2. Draw Event Markers on COM curve
    hs_plotted, to_plotted = False, False
    for f in cycle_frames:
        fidx = f.get("frame_idx", 0)
        lm = f["landmarks"]
        hip_pt = get_xy(lm.get("LEFT_HIP") or lm.get("RIGHT_HIP"))
        if not hip_pt:
            continue
        cx = hip_pt[0] - x_offset
        cy = hip_pt[1]

        if fidx in left_hs or fidx in right_hs:
            ax.plot(cx, cy, marker="s", markersize=8, markeredgewidth=1.8,
                    markerfacecolor="white", markeredgecolor="#e41a1c",
                    label="Heel-contact" if not hs_plotted else "", zorder=15)
            hs_plotted = True

        if fidx in left_to or fidx in right_to:
            ax.plot(cx, cy, marker="<", markersize=8, markeredgewidth=1.8,
                    markerfacecolor="white", markeredgecolor="#e41a1c",
                    label="Toe-off" if not to_plotted else "", zorder=15)
            to_plotted = True

    all_drawn_xs = []

    # 3. Draw Strobe Stick Figures
    for i, f_idx in enumerate(strobe_indices):
        f = cycle_frames[f_idx]
        lm = f["landmarks"]
        is_key_frame = (i == 0 or i == len(strobe_indices) - 1 or f.get("frame_idx") in left_hs or f.get("frame_idx") in left_to)

        ls = "-" if is_key_frame else "--"
        alpha = 1.0 if is_key_frame else 0.45
        lw_lead = 2.4 if is_key_frame else 1.4
        lw_trail = 1.6 if is_key_frame else 1.0
        c_lead = "#000000" if is_key_frame else "#444444"
        c_trail = "#555555" if is_key_frame else "#888888"

        def adj(pt_name):
            p = get_xy(lm.get(pt_name))
            if p:
                all_drawn_xs.append(p[0] - x_offset)
                return (p[0] - x_offset, p[1])
            return None

        nose = adj("NOSE")
        l_sh, r_sh = adj("LEFT_SHOULDER"), adj("RIGHT_SHOULDER")
        l_hip, r_hip = adj("LEFT_HIP"), adj("RIGHT_HIP")
        l_knee, r_knee = adj("LEFT_KNEE"), adj("RIGHT_KNEE")
        l_ank, r_ank = adj("LEFT_ANKLE"), adj("RIGHT_ANKLE")
        l_heel, r_heel = adj("LEFT_HEEL"), adj("RIGHT_HEEL")
        l_toe, r_toe = adj("LEFT_FOOT_INDEX"), adj("RIGHT_FOOT_INDEX")

        sh = ((l_sh[0]+r_sh[0])/2, (l_sh[1]+r_sh[1])/2) if l_sh and r_sh else (l_sh or r_sh)
        hip = ((l_hip[0]+r_hip[0])/2, (l_hip[1]+r_hip[1])/2) if l_hip and r_hip else (l_hip or r_hip)
        head_top = (sh[0], sh[1] + 0.22) if sh else nose

        def draw_seg(p1, p2, col, width, style):
            if p1 and p2:
                ax.plot([p1[0], p2[0]], [p1[1], p2[1]], color=col, linewidth=width, linestyle=style, alpha=alpha, zorder=5)

        if head_top and sh:
            draw_seg(head_top, sh, c_lead, lw_lead, ls)
        if sh and hip:
            draw_seg(sh, hip, c_lead, lw_lead + 0.5, ls)
        if head_top:
            circle = plt.Circle((head_top[0], head_top[1]), 0.025, color=c_lead, fill=True, alpha=alpha, zorder=6)
            ax.add_patch(circle)

        # Lead Leg
        draw_seg(hip, l_knee, c_lead, lw_lead, ls)
        draw_seg(l_knee, l_ank, c_lead, lw_lead, ls)
        draw_seg(l_ank, l_heel, c_lead, lw_lead, ls)
        draw_seg(l_heel, l_toe, c_lead, lw_lead, ls)
        draw_seg(l_ank, l_toe, c_lead, lw_lead * 0.9, ls)

        # Trail Leg
        if visible_side == "both":
            ls_trail = "--" if is_key_frame else ":"
            draw_seg(hip, r_knee, c_trail, lw_trail, ls_trail)
            draw_seg(r_knee, r_ank, c_trail, lw_trail, ls_trail)
            draw_seg(r_ank, r_heel, c_trail, lw_trail, ls_trail)
            draw_seg(r_heel, r_toe, c_trail, lw_trail, ls_trail)
            draw_seg(r_ank, r_toe, c_trail, lw_trail * 0.9, ls_trail)

    # 4. Ground Baseline
    min_x = min(all_drawn_xs) - 0.2 if all_drawn_xs else -0.3
    max_x = max(all_drawn_xs) + 0.25 if all_drawn_xs else 1.2
    ax.plot([min_x, max_x], [0, 0], color="#222222", linestyle="--", linewidth=1.1, alpha=0.85, zorder=2)

    # 5. Styling & Tight Framing
    ax.set_aspect("equal")
    ...
    ax.set_xlabel("Step length (m)", fontsize=12, fontweight="bold", labelpad=8)
    ax.set_ylabel("Height (m)", fontsize=12, fontweight="bold", labelpad=8)
